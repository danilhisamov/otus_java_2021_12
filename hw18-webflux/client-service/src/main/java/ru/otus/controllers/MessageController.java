...
public class MessageController {
    ...
    public void getMessage(@DestinationVariable String roomId, Message message) {
        ...
        if (specialRoom.equals(roomId)) {
            logger.warn("Message cannot be sent from room {}", specialRoom);
            return;
        }
        ...
        var msgStr = HtmlUtils.htmlEscape(message.messageStr());
        template.convertAndSend(String.format("%s%s", TOPIC_TEMPLATE, roomId),
                new Message(msgStr));
        template.convertAndSend(String.format("%s%s", TOPIC_TEMPLATE, specialRoom),
                new Message(msgStr));
    }
